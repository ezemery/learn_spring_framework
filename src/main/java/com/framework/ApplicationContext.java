...
public class ApplicationContext {
    ...
    private <T> Class<T> getImplementationForInterface(final Class<T> interfaceClass) {
        ...
        if (classes.size() > 1) {
            throw new RuntimeException("There are more than 1 implementations for interface: " + interfaceClass.getName());
        }

        return (Class<T>) classes.stream()
                .findFirst()
                .orElseThrow(() ->
                        new RuntimeException("Cannot find an implementation for interface: " + interfaceClass.getName()));
    }
    ...
    private <T> Constructor<T> getConstructor(final Class<T> clazz) {
        ...
        if (autowiredConstructors.size() > 1) {
            throw new RuntimeException("There are more than 1 autowired constructors in class: " + clazz.getName());
        }

        return autowiredConstructors.stream()
                .findFirst()
                .orElseThrow(() -> new RuntimeException("Cannot find an autowired constructor in class: " + clazz.getName()));
